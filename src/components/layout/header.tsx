...
const LOCALES = [{ code: "zh", label: "中文" }];
    ...
    <header className="sticky top-0 z-50 border-b border-[var(--color-border)] bg-[var(--color-bg)]/80 backdrop-blur-sm">
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link href={`/${locale}`} className="text-lg font-bold">
          思政编码坊
          <span className="ml-2 text-sm font-medium text-zinc-400 dark:text-zinc-500">ThinkCode</span>
        </Link>
        ...
        <nav className="hidden items-center gap-6 md:flex">
          ...
          {/* Locale switcher (hidden when only one locale is available) */}
          {LOCALES.length > 1 && (
